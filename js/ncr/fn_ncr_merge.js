...
//		scaleQusArry += ',해당 프로그램이 계획한대로 진행되었고 소통이 원활하였습니까?';
...
//			openQusArry += ',이 프로그램에서 자신에게 도움이 되었던 내용은 무엇입니까?';
			...
			scaleQusArry = 'Were you satisfied with the program overall?';
			scaleQusArry += ',Do you think the program was beneficial and helped you grow?';
			scaleQusArry += ',Was the program carried out as planned and communication was smooth?';
			scaleQusArry += ',Would you recommend the program to other students?';
			scaleQusArry += ',this program helped achieve the purpose of participation';
		}
...
//		}
	...
	if (satRschType == "peer") {
		if ( locale == 'ko' ) {
			...
			openQusArry = "[Expression] Did you use sentences with clear meaning and correct grammar, such as spelling and spacing?"
				+",[Composition] Did each paragraph of the article develop logically?"
				+",[Content] Is the topic clear and the content impressive and deep?";
		}
	}

	fnSatisOnOffAjax(satRschType, obj, scaleQusArry, openQusArry,choiceQusArry, choiceAnsQusArry, locale);
}
...
/**tr 추가**/
function addBudget() {
	var curLocale = $("#admin_locale").val();
	...
	if ($tableTr.find("tbody tr").length > 5) {
		if(curLocale == 'en') {
			alert("You can add up to five budgets.");
		} else {
	...
	var html = '<tr id="budgetTr_'+trIndex+'">';
		html +=		'<td>';
		html += 		'<input type="hidden" name="budgetTrCnt" id="budgetTrCnt_'+trIndex+'" value="1">';
		html +=			'<select class="form-control input-sm wd_p30 mg_r5 dp_inline" name="nbiBudgetCd" id="nbiBudgetCd" title="예산 대분류" data-validation=\'{"required" : true}\' onchange="fnCreateBudgetEtc(this);">';
		if(curLocale == 'en') {
			html += 			'<option value="">Please select budget type.</option>';
			html += 			'<option value="NCR_BUD_4100">School budget</option>';
			html += 			'<option value="NCR_BUD_4200">Treasury</option>';
			html += 			'<option value="NCR_BUD_9999">Other</option>';
		} else {
			html += 			'<option value="">예산구분을 선택해주세요.</option>';
			html += 			'<option value="NCR_BUD_4100">교비</option>';
			html += 			'<option value="NCR_BUD_4200">국고</option>';
			html += 			'<option value="NCR_BUD_9999">기타</option>';
		}
		html += 		'</select>'
		html += 		'<input type="text" class="form-control wd_p30 input-sm dp_inline" onkeyup="chkByte(this, 1000)" name="nbiBudgetMainEtc" id="nbiBudgetMainEtc" title="대분류 기타사항" value="" readonly>'
		html += 	'</td>'
//		html += 	'<td>'
//		html += 		'<div class="">'
//		html += 			'<select class="form-control input-sm wd_p80 dp_inline" name="nbiBudgetSubCd" id="nbiBudgetSubCd" title="예산 항" onchange="fnBiBudgetCdSelChg(this, \'sub\');" data-validation=\'{"required" : true}\' >'
//		html += 				'<option value="">예산구분을 선택해주세요.</option>'
//		html += 			'</select>'
//		html += 		'</div>'
//		html += 	'</td>'
//		html += 	'<td>'
//		html += 		'<div class="">'
//		html += 			'<select class="form-control input-sm wd_p80 dp_inline" name="nbiBudgetThirdCd" id="nbiBudgetThirdCd" title="예산 목" onchange="fnBiBudgetCdSelChg(this, \'third\');" data-validation=\'{"required" : true}\' >'
//		html += 				'<option value="">예산구분을 선택해주세요.</option>'
//		html += 		'</select>'
//		html += 		'</div>'
//		html += 	'</td>'
//		html += 	'<td>'
//		html += 		'<div class="" id="budgetForthCdDiv_0">'
//		html += 			'<select class="form-control input-sm wd_p80 dp_inline" name="nbiBudgetForthCd" id="nbiBudgetForthCd" title="예산 세목" data-validation=\'{"required" : true}\' >'
//		html += 				'<option value="">예산구분을 선택해주세요.</option>'
//		html += 			'</select>'
//		html += 		'</div>'
//		html += 	'</td>'
		html += 	'<td>'
		html += 		'<div class="" id="budgetPriceDiv_0">'
		html += 			'<input type="text" class="form-control wd_p70 ta_r dp_inline mg_r5" name="nbiBudgetPrice" id="nbiBudgetPrice" maxlength="11" title="예산액" onkeyup="commaKeyUpChkSum(this);" data-validation=\'{"required" : true}\' >'
//		html += 			'<button class="btn btn-sm btn-primary" style="margin-bottom: 3px;" type="button" onclick="addSubBudget(this,\''+trIndex+'\');"><i class="fa fa-plus mg_r5"></i>추가</button>'
		html += 		'</div>'
		html += 	'</td>'
		html += '</tr>'
	...
	$tableTr.find("tbody").append(html);
	$tableTr.find("tbody").append(
				'<tr>'
					+ '<th scope="row">' + (curLocale == 'en' ? " Total" : " 합계" ) + '</th>'
						+ '<td class="ta_c" id="budgetSum">'
						+ budgetSum + (curLocale == 'en' ? " won" : " 원" ) + '</td>'
				+ '</tr>');
...
function addPartDepart(npiPartiDeptCd, npiPartiDeptNm) {
	...
	if (_rowCount > 9) {
		if(locale == 'en') {
			alert("You may add up to 10 participating departments.");
		}else {
	...
	departTag += '<input type="text" class="form-control input-sm" name="npiPartiDeptNm" onkeydown="fnEnterActionForDepartTd('+ "'npiPartiDept','"+ _rowCount+"'"+');"'+'id="npiPartiDeptNm_'+_rowCount+'" placeholder="'+(locale == "en" ? "Participating departments" : "참여학과")+'" title="참여학과" value="'+npiPartiDeptNm+'" readonly>';
	departTag += '<button type="button" class="btn btn-primary btn_form btn_search btn-xs" onclick="fnGetDeptInfo('+ "'npiPartiDept','"+ _rowCount+ "'"+ ');"><i class="fa fa-search"></i>'+ (locale == "en" ? "Search" : "검색")+'</button>';
 ...
 ****************************************
 **/
function fnLeaderShipGoRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/leaderShipProgramListSTF/r/m/getProgramRecruitList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnPreUnistarGoRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/preUnistarListSTF/r/m/getProgramRecruitList.do";
	$('#detailForm').attr("action",_url).submit();
}
function fnMySchoolGoRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/mySchoolProgramListSTF/r/m/getProgramRecruitList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnCaMentorShipRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/caMentoringStateListSTF/r/m/getProgramRecruitList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnTutoringGoRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/tutoringListSTF/r/m/getProgramRecruitList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnVolunteerGoRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/volunteerListSTF/r/m/getProgramRecruitList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnTutoringTutorRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

	var _url = BASE_PATH+"/tutoringTutorListSTF/r/m/getProgramTutorList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnMentoringMentorRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);

//	var _url = BASE_PATH+"/caMentoringMentorListSTF/r/m/getProgramMentorList.do";
	var _url = BASE_PATH+"/caMentoringStateListSTF/r/m/getProgramMentorList.do";
	$('#detailForm').attr("action",_url).submit();
}

function fnRecruitList(_keyId) {
	$('#detailForm #npiKeyId').val(_keyId);
...
/**
*********************************************************
@ function : ncr 프로그램 불러오기 팝업
@ comment  :
@ history  : 2019-02-18 (최초작성)
**********************************************************
**/
function fnGetNcrProgramPopUp(_params, contentsId, menuName){
	var _url = basePath + "/cmm/fms/getProgramListPopUp.do";
	var _popupParams = {
						 menuName
						 , contentsId
						};
	loadPopup("#layerPopup", _url, _popupParams);
	$("#mymodal").modal();
}
