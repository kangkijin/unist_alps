var currLocale="";
var firstQuestionDel;
var firstQuestionChange;
var lastQuestionChange;
var addNot;
var atLeastOne;
/*
 * 만족도 조사 기능 시작
 */
function fnSatisOnOffAjax(satRschType, obj, scaleQusArry, openQusArry, choiceQusArry, choiceAnsQusArry, locale) {
	...
	if ( currLocale == 'en' ) {
		firstQuestionDel ='The first question cannot be deleted';
		firstQuestionChange = 'Since this is the first question, the order cannot be changed.';
		lastQuestionChange = 'Since this is the last question, the order cannot be changed.';
		addNot = 'can\'t add more';
		atLeastOne = 'At least one answer is required.';
	} else {
		firstQuestionDel ='첫 질문은 삭제할 수 없습니다.';
		firstQuestionChange = '첫 질문이여서 순서를 변경할 수 없습니다.';
		lastQuestionChange = '마지막 질문이여서 순서를 변경할 수 없습니다.';
		addNot = '더 이상 추가가 불가합니다.';
		atLeastOne = '적어도 하나 이상의 답변이 필요합니다.';
	}
	
	if ( locale == 'ko' ) {
		str = "미사용으로 하시면 입력하신 문항내용이 모두 지워집니다. 미사용으로 변경하시겠습니까?";
	} else {
		str = "If you do not use it, all the contents of the entered items will be deleted. Do you want to change it to Unused?";
	}
...
function addQus(type) {
	...
	if(currLocale == "en") {
		scaleNm = "5-point scale";
		openNm = "short-answer question";
		choiceNm = "freeform Multiple Choice";
	}else {
		scaleNm = "5점척도";
		openNm = "주관식";
		choiceNm = "자유형 객관식";	
		...
		$tableTr.find("tbody").append("<tr>"
				...
				+"<td class='td_input'>"
					+"<select class='form-control input-sm' name='sqmTypCd1' onchange='changeType(this, \"std\");'>"
						+"<option value='SCALE'>"+ scaleNm +"</option>"
						+"<option value='OPEN'>"+ openNm +"</option>"
						+"<option value='CHOICE'>"+ choiceNm +"</option>"
					+"</select>"
...
function addAns(obj, type) {
	...
	if(type == "peer") {
		...
		$(obj).parent("div").next().append("<li class='ta_l mg_t5'>"
				+"<input type='text' class='form-control input-sm dp_inline wd_p90 va_m' name='samCont1' title='답변 내용'>"
				+"<button type='button' class='btn btn-danger btn-danger' onclick='removeAns(this, \"peer\");' title='질문삭제'><i class='fa fa-minus mg_r0'></i></button>"
			+"</li>");
		// 답변 개수 수정
