...
//	console.log( this )
	...
	dropZones.each(function() {
		...
		if( canDragDrop() ) {
			var htmlDesc = "";
			if($("#adminAttLocale").val() == 'en') {
				htmlDesc = "<dt>Click the button at the top or drag a file to add it.</dt>";
			}else{
				htmlDesc = "<dt>상단의 버튼을 클릭 하거나 또는 파일 Drag로 추가해주세요.</dt>";
			}
			"";
			htmlDesc += "<dd>";
...
//				console.log( elements[i].tagName + " : " + elements[i].name );
		...
		, invalidHandler : function(form, validator) {
			...
			if ( $("#admin_locale").val() == 'en' ) {
				alert('Please check your input');
			} else {
				if($($(validator.errorList)[0].element).attr('type') == "checkbox") {
					alert($($(validator.errorList)[0].element).parent().parent().attr('title') + '을(를) 확인하세요.');
				}else {
					alert($($(validator.errorList)[0].element).attr('title') + '을(를) 확인하세요.');
				}
		...
		success: function(r) {
			if (r.sustMdCdList.length > 0) {
				for(var idx=0; idx<r.sustMdCdList.length; idx++) {
					...
					if(typeof selValue != "undefined" && selValue == obj[cdCol]) {
						targetView.append("<option value='"+obj[cdCol]+"' selected>"+obj.deptNmKo+"</option>");
					} else {
						targetView.append("<option value='"+obj[cdCol]+"'>"+obj.deptNmKo+"</option>");
					}
				}
			}
		},
...
/**
 * 공통코드 조회 Value 값 retuen
 * @param comnCdPcodeId	= 조회할 부모 코드(upCd)
 */
function fnSetComnCdVal(comnCdPcodeId) {
    var _url = basePath + "/cmm/fms/getCodeListAjax.do";
    if (comnCdPcodeId == "") {
        return Promise.reject("comnCdPcodeId is empty");
    }
        ...
        jQuery.ajax({
            ...
            success: function (r) {
                if (r.beanlist.length > 0) {
                    ...
                    reject("beanlist is empty");
                }
            },
            error: function (r) {
                reject("Failed to get code list");
            }
        });
    });
}
 ...
 * @param comnCdPcodeId	= 조회할 부모 코드(upCd)
 */
function fnSetSbjComnCdVal(comnCdPcodeId) {
    var _url = basePath + "/cmm/fms/getCodeListAjax2.do";
    if (comnCdPcodeId == "") {
        return Promise.reject("comnCdPcodeId is empty");
    }
...
/**
 * 공통단과대학 조회 Value 값 retuen
 * @param toporg = 조회할 부모 코드(toporg)
 */
function fnSetColgCdVal(toporg) {
    var _url = basePath + "/cmm/fms/getColgListAjax.do";
    if (toporg == "") {
        return Promise.reject("toporg is empty");
    }
    ...
    return new Promise(function (resolve, reject) {
        jQuery.ajax({
            ...
            success: function (r) {
                if (r.colgList.length > 0) {
                    var obj = r.colgList;
                    resolve(obj);
                } else {
                    reject("colgList is empty");
                }
            },
            error: function (r) {
                reject("Failed to get code list");
...
/**
 * 공통학부 조회 Value 값 retuen
 * @param scId = 조회할 부모 코드(scId)
 */
function fnSetSustCdVal(scId) {
    var _url = basePath + "/cmm/fms/getSustListAjax.do";
    if (scId == "") {
        return Promise.reject("scId is empty");
    }
    ...
    return new Promise(function (resolve, reject) {
        jQuery.ajax({
            ...
            success: function (r) {
                if (r.sustList.length > 0) {
                    var obj = r.sustList;
                    resolve(obj);
                } else {
                    reject("sustList is empty");
                }
            },
            error: function (r) {
                reject("Failed to get code list");
 ...
 * 졸업이수요건 HTML
 * @param scId = 조회할 부모 코드(scId)
 * @param menuDiv = div Id값(menuDiv)
 */
function fnSetGreduHtml(scId,menuDiv) {
    var _url = basePath + "/cmm/fms/getGreduHtmlAjax.do";
    return new Promise(function (resolve, reject) {
        jQuery.ajax({
            type: "POST",
            url: _url,
...
/**
 * 학과 교과목 리스트 HTML
 * @param scId = 학과 코드(scId)
 * @param cgId = 전공 코드(cgId)
 * @param menuDiv = div Id값(menuDiv)
 * @param year = 연도 코드(year)
 * @param term = 학기 코드(term)
 */
function fnSetSubjectHtml(scId, cgId, menuDiv , year, term,_pageNo, _menuIndex, _menuType, _searchKeyword) {
    var _url = basePath + "/cmm/fms/getSubjectHtml.do";
       if (!_pageNo || typeof _pageNo === 'undefined' || _pageNo === null || _pageNo === '') {
		  _pageNo = 1;
		}
    return new Promise(function (resolve, reject) {
        jQuery.ajax({
 ...
 * @param term = 학기 코드(term)
 */
function getSubjectData(scId, cgId, year, term) {
    var _url = basePath + "/cmm/fms/getSubjectData.do";
    return new Promise(function (resolve, reject) {
        jQuery.ajax({
            type: "POST",
            url: _url,
			...
			success: function (r) {
                if (r.sustList.length > 0) {
                    var obj = r.sustList;
                    $("#totalDiv").append("<spen>Total</spen>"+r.sustList.length);
                    resolve(obj);
                } else {
                    reject("sustList is empty");
                }
...
function fnGoUrlPopup(seId,year,term,stdNo){
	var url = basePath + "/cmm/fms/getEvUrlPopup.do";
    return new Promise(function (resolve, reject) {
        jQuery.ajax({
            type: "POST",
            url: url,
			...
			success: function (r) {
                if (r.EV_URL != null) {
					var name = "popup";
					var option = "width=1200,height=900";
					window.open(r.EV_URL, name, option);
                } else {
                    reject("Failed to get data");
                }
            },
...
fnGoDetailPopup = function(smId){
	// 교과목정보 상세보기 팝업띄우기
	$.ajaxSetup({cache:false});
 	var popupDivId = "#layerPopup";
	var popupUrl = basePath + "/majorSubj/a/m/recentStatusPopup.do";
	var popupParams = {
		 	smId : smId
	 	}; // 팝업 호출시의 파라미터
	clientLoadPopup (popupDivId, popupUrl, popupParams);
